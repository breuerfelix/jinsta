...
*/
async function storyView(client: IgApiClient, userIds: number[], lastSeenPerUser?: any): Promise<void> {
	try{
		...
		if(lastSeenPerUser){
			...
			stories = stories.filter(
				({ taken_at, user: { pk } }) => !lastSeenPerUser[pk] || taken_at > lastSeenPerUser[pk]
			);
		}
...
*/
async function storyMassView(client: IgApiClient, config: Config): Promise<void> {
	...
	try {
		...
		const userIds: number[] = [], lastSeenPerUser: any = {};
		storyTrayItems.map(
			({ seen, user: { pk } }) => {lastSeenPerUser[pk] = seen; userIds.push(pk)}
		);
